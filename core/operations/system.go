...
func (s *System) initializeHealthCheckHandler() {
	...
	s.mux.Handle("/healthz", s.handleChain(s.healthHandler, false))
}

func (s *System) initializeLoggingHandler() {
	s.mux.Handle("/logspec", s.handleChain(httpadmin.NewSpecHandler(), s.options.TLS.Enabled))
}
...
func (s *System) initializeMetricsProvider() {
	...
	case "prometheus":
		...
		s.versionGauge = versionGauge(s.Provider)
		s.mux.Handle("/metrics", s.handleChain(promhttp.Handler(), s.options.TLS.Enabled))
...
func (s *System) initializeVersionInfoHandler() {
	...
	s.mux.Handle("/version", s.handleChain(versionInfo, false))
}

func (s *System) handleChain(h http.Handler, secure bool) http.Handler {
	if secure {
